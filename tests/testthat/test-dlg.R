test_that("checks if R is run under JGR or RStudio", {
    expect_identical(.is_jgr(), "package:JGR" %in% search())
    expect_identical(.is_rstudio(), rstudioapi::isAvailable())
  })

test_that("manipulates data in a temporary environment", {
...
test_that("dlg_dir works in non-interactive sessions", {
  wd <- getwd()
